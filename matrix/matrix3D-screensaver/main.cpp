...
void main() {
    ...
    // glyphs live in scroll space: they fall as t grows
    float y   = st.y + t;
    float row = floor(y);
    float fy  = fract(y);

    // trail head in SCREEN space: falls, then wraps top
    float trail = mix(4.0, 10.0, hc2);
    float wrap  = u_grid.y + trail;
    float head  = u_grid.y - mod(t, wrap);
    float depth = head - st.y;                 // 0 at head, grows down the tail
    float lit   = step(0.0, depth) * step(depth, trail);
    float b     = 1.0 - depth / max(trail, 0.001);
    b *= b;
    b *= mix(0.35, 1.0, hash(vec2(col, row) * 1.93));

    // glyph per cell, with occasional flicker re-roll
...
if (dot(N, V) < 0.0) N = -N;   // two-sided: tolerate flipped winding
    ...
    col += rim  * vec3(0.10, 0.90, 0.35) * 0.35;          // was *0.55

    float band = 0.5 + 0.5 * sin(v_p.y * 14.0 - u_time * 1.5);   // soft sine bands
    col += band * band * vec3(0.03, 0.22, 0.08) * 0.25;          // was hard smoothstep stripes
    ...
    float scan = 0.92 + 0.08 * sin(gl_FragCoord.y * 1.2);        // was 0.88+0.12*sin(y*2.4)
    vec2 q = gl_FragCoord.xy / u_res * 2.0 - 1.0;
    float vig = 1.0 - 0.35 * dot(q * 0.7, q * 0.7);
    col *= scan * vig;
...
// Compute normals if the file didn't supply them. Uses crease-angle
// clustering: faces meeting sharper than ~40 degrees get separate
// normals (hard edges -> cubes stay cubes), gentle angles blend
// (spheres/cylinders stay smooth).
if (missingNormal) {
    const float CREASE_COS = cosf(40.0f * 3.14159265f / 180.0f);

    size_t nf = verts.size() / 3;
    std::vector<V3> fn(nf);
    for (size_t fi = 0; fi < nf; ++fi) {
        const Vertex& a = verts[fi*3];
        const Vertex& b = verts[fi*3 + 1];
        const Vertex& c = verts[fi*3 + 2];
        V3 ab(b.px - a.px, b.py - a.py, b.pz - a.pz);
        V3 ac(c.px - a.px, c.py - a.py, c.pz - a.pz);
        fn[fi] = cross3(ab, ac);          // area-weighted face normal
    }
    
    
// Orient computed face normals outward (away from mesh centroid) and
// flip triangle winding where needed, so the geometry self-repairs
// even if the file's faces are wound backwards.
V3 bmin(1e30f,1e30f,1e30f), bmax(-1e30f,-1e30f,-1e30f);
for (auto& v : verts) {
    bmin.x = std::min(bmin.x, v.px); bmin.y = std::min(bmin.y, v.py); bmin.z = std::min(bmin.z, v.pz);
    bmax.x = std::max(bmax.x, v.px); bmax.y = std::max(bmax.y, v.py); bmax.z = std::max(bmax.z, v.pz);
}
V3 bc = (bmin + bmax) * 0.5f;
for (size_t fi = 0; fi < nf; ++fi) {
    V3 a(verts[fi*3].px,   verts[fi*3].py,   verts[fi*3].pz);
    V3 b(verts[fi*3+1].px, verts[fi*3+1].py, verts[fi*3+1].pz);
    V3 c(verts[fi*3+2].px, verts[fi*3+2].py, verts[fi*3+2].pz);
    V3 fc = (a + b + c) * (1.0f/3.0f);
    if (dot3(fn[fi], fc - bc) < 0.0f) {     // normal points toward centroid -> flip
        std::swap(verts[fi*3+1], verts[fi*3+2]);
        fn[fi] = fn[fi] * -1.0f;
    }
}


    // group corners by identical position
    std::map<std::tuple<float,float,float>, std::vector<int>> byPos;
    for (size_t i = 0; i < verts.size(); ++i)
        byPos[std::make_tuple(verts[i].px, verts[i].py, verts[i].pz)].push_back((int)i);

    for (auto& kv : byPos) {
        struct Cluster { V3 sum; std::vector<int> members; };
        std::vector<Cluster> clusters;
        for (int ci : kv.second) {
            V3 f = norm3(fn[ci / 3]);
            int best = -1;
            for (size_t k = 0; k < clusters.size(); ++k)
                if (dot3(norm3(clusters[k].sum), f) > CREASE_COS) { best = (int)k; break; }
            if (best < 0) { clusters.push_back(Cluster()); best = (int)clusters.size() - 1; }
            clusters[best].sum = clusters[best].sum + f;
            clusters[best].members.push_back(ci);
        }
        for (auto& cl : clusters) {
            V3 n = norm3(cl.sum);
            for (int ci : cl.members) {
                verts[ci].nx = n.x; verts[ci].ny = n.y; verts[ci].nz = n.z;
            }
        }
    }
}
...
// procedural fallback: trefoil knot, parallel-transport tube with
// holonomy correction so the closed curve joins without a seam/spike
static void makeKnot(std::vector<Vertex>& out) {
    const int U = 320, W = 28;
    const float tube = 0.36f, TAU = 6.28318530718f;
    auto kp = [&](float t) {
        float r = 2.0f + cosf(3.0f * t);
        return V3(r * cosf(2.0f * t),
                  sinf(3.0f * t) * 1.3f,
                  r * sinf(2.0f * t)) * 0.55f;
    };
    ...
    for (int i = 0; i <= U; ++i) {
        ...
        V3 n = (i == 0) ? norm3(cross3(tg, V3(0, 1, 0)))
                        : norm3(prevN - tg * dot3(prevN, tg));   // project => parallel transport
        C[i] = kp(t); Nr[i] = n; Br[i] = cross3(tg, n);
        prevN = n;
    }

    // The frame returns rotated by the holonomy angle 'a'. Spread -a
    // evenly along the whole tube so ring U matches ring 0 exactly.
    float a = atan2f(dot3(Nr[U], Br[0]), dot3(Nr[U], Nr[0]));
    for (int i = 0; i <= U; ++i) {
        float phi = -a * (float)i / (float)U;
        float cs = cosf(phi), sn = sinf(phi);
        V3 n2 = Nr[i] * cs + Br[i] * sn;
        V3 b2 = Br[i] * cs - Nr[i] * sn;
        Nr[i] = n2; Br[i] = b2;
    }
